...
void loop() {
  ...
  if ((ir[0] && ir[1] && ir[2]) || (ir[5] && ir[6] && ir[7]) || 
...
void LSR() {    // for junction points
  ...
  else if (irr[0] && irr[1] && irr[2]) {              // case '4' or case'6'  and one more hiden case to be find!
...
//    Serial.println(readLength);
  ...
  else if (digitalRead(ir1) && digitalRead(ir2) && digitalRead(ir4) && digitalRead(ir6) && digitalRead(ir8)){
    done();
  }
